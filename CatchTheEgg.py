...
class Egg:
    ...
    def __init__(self, canvas, basket):
        ...
        self.score = 0
        self.score_text = canvas.create_text(150,100, anchor=W, fill='royalblue', font=("Arial", 16), text=".")
...
class Basket:
    ...
    def move_busket(self):
        self.canvas.move(self.rect, self.x, 0)
        self.canvas.image(self.rect, self.x, 0)
        pos = self.canva.coords(self.rect)
        if pos[0] <= 0:
            eslf.x = 0
        if pos[2] >= 500:
            self.x = 0
...
canvas.pack()
'''bg = PhotoImage(file="C:/Program Files (x86)/bg.png")
canvas.create_image(0, 0, anchor=NW, image=bg)
basketimage = PhotoImage(file="C:/Program Files (x86)/basket.png")'''
